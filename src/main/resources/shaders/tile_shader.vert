...
layout (location = 2) in vec2 aInstancePos; //Position of the instance
...
void main() {
    vec3 scaled = vec3(aPos, 0f) * vec3(scale, 1f);
    vec3 translated = scaled + vec3(aInstancePos, 0.f) + vec3(offset, 0.f);
